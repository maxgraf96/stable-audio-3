...
mx::array StandardSelfAttention::operator()(const mx::array& x) const {
    const auto& s = x.shape();
    const int B = s[0], T = s[1];
    constexpr int H = NUM_HEADS, D = HEAD_DIM;

    // Standard 3-way QKV split (medium does 5-way for differential attn).
    mx::array qkv = mx::matmul(x, mx::transpose(to_qkv));  // (B, T, 3*EMBED)
    ...
    // Partial RoPE on first 32 of 64 head dims (same as medium)
    ...
    // Single SDPA — no diff subtraction
    mx::array out = mx::fast::scaled_dot_product_attention(q, k, v, ATTN_SCALE);
...
// Mirrors dit_mlx.py:CrossAttention — to_q (EMBED→EMBED), to_kv splits
// context into K/V, single SDPA, NO RoPE on cross-attn Q/K.
mx::array StandardCrossAttention::operator()(
    const mx::array& x, const mx::array& context) const
{
    ...
    mx::array q  = mx::matmul(x, mx::transpose(to_q));                       // (B, Tx, EMBED)
    auto kv_parts = mx::split(mx::matmul(context, mx::transpose(to_kv)),     // 2× (B, Tc, EMBED)
                              2, -1);
    ...
    mx::array qh = to_q_heads(q);
    mx::array kh = to_c_heads(kv_parts[0]);
    mx::array vh = to_c_heads(kv_parts[1]);

    qh = mx::fast::rms_norm(qh, q_norm_w, QK_NORM_EPS);
    kh = mx::fast::rms_norm(kh, k_norm_w, QK_NORM_EPS);

    mx::array out = mx::fast::scaled_dot_product_attention(qh, kh, vh, ATTN_SCALE);
...
// Identical math to medium — only the inner dim (4096 vs 6144) and the
// embed dim (1024 vs 1536) differ; shape info comes from the tensors.
...
// ── TransformerBlock ─────────────────────────────────────────────────
// Same residual / norm / scale-shift-gate structure as medium. The only
// difference is the attention type the struct holds.
    ...
    const mx::array& local_emb_padded) const
{
    ...
    mx::array ss = to_scale_shift_gate + global_cond;
    ...
    const mx::array& local_add_cond) const
{
    ...
    mx::array mem = mx::broadcast_to(
        mx::expand_dims(memory_tokens, 0),
        {B, NUM_MEMORY_TOKENS, EMBED_DIM});
    x = mx::concatenate({mem, x}, 1);
    ...
    for (const auto& layer : layers) {
        mx::array local_emb = layer.to_local_embed(local_add_cond);
    ...
    return mx::matmul(x, mx::transpose(project_out_w));
}
...
static ExpoFourierFeatures make_timestep_features(mx::Dtype /*dt*/) {
    ...
    mx::eval(freqs);
    // Keep freqs at fp32 regardless of model dtype — matches Python's
    // mx.linspace default (fp32), and downstream promotes correctly.
    ...
    const std::optional<mx::array>& local_add_cond) const
{
    ...
    mx::array x_lc = mx::transpose(x, {0, 2, 1});
    ...
    mx::array h = transformer(x_pp, context, global_embed, local);
    ...
    mx::Dtype dtype)
{
    ...
    for (int i = 0; i < DEPTH; ++i) {
        ...
        ct.layers.push_back(TransformerBlock{
            ...
            /*self_attn=*/ StandardSelfAttention{
