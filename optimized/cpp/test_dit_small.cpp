// Sanity check for sa3::dit_small — load the sm-music safetensors, run a
// forward at fp32 with deterministic inputs, dump shape + stats. Catches
// loader-key mismatches and shape bugs before we sink time into SAME-S.
...
int main(int argc, char** argv) {
    if (argc != 2) {
        std::cerr << "usage: " << argv[0]
                  << " <path/to/dit_sm-music_f16.safetensors>\n";
        return 2;
    }
    ...
    auto dit_model = sa3::dit_small::load_dit(tensors, T_lat, mx::float32);
    ...
    mx::array x     = make({1, sa3::dit_small::IO_CHANNELS, T_lat});
    mx::array tval  = mx::array({0.5f});                                  // (1,)
    mx::array cross = make({1, 257, sa3::dit_small::COND_TOKEN_DIM});
    mx::array gcond = make({1, sa3::dit_small::GLOBAL_COND_DIM});
