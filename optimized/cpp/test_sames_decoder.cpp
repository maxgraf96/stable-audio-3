// Bit-exactness test for the C++ SAME-S decoder. Compare vs verify_sames_decoder.py.
//   ./test_sames_decoder <same_s_decoder_safetensors> [T_lat]
#include <algorithm>
#include <cmath>
#include <cstdio>
#include <string>
...
int main(int argc, char** argv) {
    ...
    auto dec = sa3::sames::load_sames_decoder(loaded.first, mx::float32);

    int total = 256 * T_lat;
    mx::array latents = mx::reshape(
        mx::arange(0, total, 1, mx::float32) / static_cast<float>(total),
        mx::Shape{1, 256, T_lat});
    auto out = dec(latents);
    dump("dec", out);
    return 0;
}
