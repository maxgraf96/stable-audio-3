...
// ── SelfAttention (standard MHA, single SDPA) ────────────────────────
mx::array SelfAttention::operator()(const mx::array& x) const {
...
// ── CrossAttention (standard, single SDPA, no RoPE) ──────────────────
mx::array CrossAttention::operator()(
    const mx::array& x, const mx::array& context) const
{
    ...
    auto kv_parts = mx::split(mx::matmul(context, mx::transpose(to_kv)), 2, -1);  // 2× (B, Tc, EMBED)
    ...
    mx::array v = to_c_heads(kv_parts[1]);
...
static ExpoFourierFeatures make_timestep_features(mx::Dtype /*dt*/) {
    ...
    mx::eval(freqs);
    // Keep freqs at fp32 regardless of DiT dtype (see dit.cpp for rationale):
    // t (fp16) * freqs (fp32) promotes to fp32, cascading global_embed and the
    // per-block scale/shift/gate into fp32. Casting freqs to fp16 breaks
    // bit-exactness against Python.
